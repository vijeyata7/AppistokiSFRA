...
var sfmcTokenServiceObj = require('app_custom_sfmc/cartridge/scripts/service/sfmcGenerateTokenAPI');
var sfmcUpsertOrderDataObj = require('app_custom_sfmc/cartridge/scripts/service/sfmcUpsertOrderDataAPI');
var sfmcOrderConfirmationObj = require('app_custom_sfmc/cartridge/scripts/service/sfmcOrderConfirmationAPI');
...
server.post(
    ...
    function (req, res, next) {
		...
		for(var i = 0; i < order.productLineItems.length ; i++) {
			var id = order.productLineItems[i].productID;
			var qty = order.productLineItems[i].quantity.value;
	        var price = order.productLineItems[i].priceValue;
	        var uniqueId = order.productLineItems[i].productID;
			var etmc = {item : id,
					quantity : qty,
					price : price,
	                unique_id : uniqueId
				};
			orderarray.push(etmc);
    	}

		
		//SFMC GENERATE TOKEN API
		//when a new customer account created/registered in sfcc then call the sfmc api to push that customer record into sfmc
		var sfmcToken = null;
        var requestBody = {};
   		requestBody.grant_type = 'client_credentials';
  		requestBody.client_id = 'cixniyxyg32c3v2szjdre4qk';
   		requestBody.client_secret = 'Dy7pKk6nF6B36ZQFrkcTuvX2';
		requestBody.account_id = '514000190';
					
		var result = sfmcTokenServiceObj.call(requestBody);
		if (result.status === 'OK') {
		sfmcToken = JSON.parse(result.object.apiResponse.text).access_token;
		}

        //SFMC PUSH ORDER API
        //sfcc calls the upsert order api of sfmc when customer places an order after token generation.
        var orderRequestBody = {};
        var date = new Date();
        var orderDate = date.getDate() + '/' + date.getMonth() + '/' + date.getYear();
        orderRequestBody.items = [{  
        	"Email" : request.getSession().getCustomer() && request.getSession().getCustomer().getProfile().getCredentials().login ? request.getSession().getCustomer().getProfile().getCredentials().login : order.customerEmail,  
            "CustomerId" : order.customerNo,
            "OrderNo" : order.orderNo,
            "CustomerName" : order.customerName,
            "Orderdate" : orderDate,
            "NetPrice" : order.totalNetPrice.value,
			"GrossPrice" : order.totalGrossPrice.value,
			"ProductId" : order.productLineItems[0].productID
        }]; 
        var status = sfmcUpsertOrderDataObj.call(orderRequestBody,sfmcToken);

        //SFCC calls the SFMC Order confirmation API when customer places an order after token generation.
		var orderConfirmationRequestBody = {};
        var date = new Date();
        var orderDate = date.getYear() + '-' + '0' + (date.getMonth()+1) + '-' + date.getDate();
        orderConfirmationRequestBody.contactKey = request.getSession().getCustomer().getProfile().getCredentials().login ? request.getSession().getCustomer().getProfile().getCredentials().login : order.customerEmail;
        orderConfirmationRequestBody.EventDefinitionKey = "APIEvent-c9579183-597a-076b-c33d-c5f2c03bda12";
        orderConfirmationRequestBody.Data = {
			"CustomerId" : order.customerNo, 
			"OrderNo" : order.orderNo,
			"CustomerName" : order.customerName, 
        	"Email" : request.getSession().getCustomer().getProfile().getCredentials().login ? request.getSession().getCustomer().getProfile().getCredentials().login : order.customerEmail,  
            "Orderdate" : orderDate,
            "NetPrice" : order.totalNetPrice.value,
			"GrossPrice" : order.totalGrossPrice.value,
			"ProductId" : order.productLineItems[0].productID,
			"ProductImage": orderModel.shipping[0].productLineItems.items[0].images.small[0].absURL,
        	"ProductName": order.productLineItems[0].productName
        }; 
        sfmcOrderConfirmationObj.call(orderConfirmationRequestBody,sfmcToken);

...
function CartModel(basket) {
    ...
    if (basket !== null) {
		...
		for(var i = 0; i < productLineItemsModel.items.length ; i++) {
			var id = productLineItemsModel.items[i].id;
			var qty = productLineItemsModel.items[i].quantity;
	        var price = productLineItemsModel.items[i].price.sales.formatted;
	        var uniqueId = productLineItemsModel.items[i].id;
			var etmc = {item : id,
					quantity : qty,
					price : price,
	                unique_id : uniqueId
